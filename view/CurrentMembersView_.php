...
class CurrentMembersView extends View
{
	function fetch()
	{
		...
		if(!empty($this->user->email))
		{
			...
			// 1 - pending
			// 4 - failed
			$orders_ = $this->orders->get_orders(array('user_id'=>$this->user->id, 'status'=>array(0,1,4), 'sort'=>'date'));
			...
			if(!empty($guest->booking) || $guest->type != 1)
			{
				...
				if((!empty($guest->room) && $guest->room->apartment_id !=0) || !empty($guest->apt) || $guest->type != 1)
				{
					...
					if(!empty($bjs))
					{
						...
						$notify_ = $this->notifications->get_notifications(array('object_id'=>array_keys($users), 'type'=>array('5', '6')));
						foreach ($notify_ as $n) 
						{
							if($n->type == 5)
							{
								...
								elseif($users[$n->object_id]->inquiry_arrive >= date_format($date_now, 'Y-m-d') && strtotime($users[$n->object_id]->inquiry_arrive) <= strtotime(date_format($date_now, 'Y-m-d'))+(9*24*60*60))
								{
									$n->active = 1;
									$notify[] = $n;
								}
							...
							elseif($n->type == 6)
							{
								...
								elseif($users[$n->object_id]->inquiry_depart >= date_format($date_now, 'Y-m-d') && strtotime($users[$n->object_id]->inquiry_depart) <= strtotime(date_format($date_now, 'Y-m-d'))+(7*24*60*60))
								{
									$n->active = 1;
									$notify[] = $n;
								}
		...
		if(!empty($this->user->house_id))
		{
			...
			if(!empty($house_laeders_))
			{
				foreach($house_laeders_ as $k=>$hl)
				{
					$house_laeders[$k] = $hl;
					if(!empty($hl->blocks))
						$house_laeders[$k]->blocks = unserialize($hl->blocks);
				}
			}
