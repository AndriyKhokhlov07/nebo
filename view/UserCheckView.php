...
class UserCheckView extends View
{
	...
	function fetch()
	{
		...
		$salesflow_id = intval($this->request->get('s'));
		$admin_view = $this->request->get('a');
				...
				if($notification->type == 2 && !empty($notification->user_id))
				{
					...
					if(!empty($user))
					{
						...
						if(!empty($salesflow_id))
							$salesflow = $this->salesflows->get_salesflows(['id'=>$salesflow_id, 'limit'=>1]);
						else
						...
						elseif(!empty($salesflow->application_data) && isset($salesflow->application_data['employment_income']) && empty($admin_view))
							$success = '?success=sended';
						else
							$success = '?';

                        if (!empty($salesflow) && empty($contract)) {
                        ...
                        if ($user_type == 'guarantor') {
                            $url_path = '/guarantor/application';
                        }

						if(!empty($contract))
							header('Location: '.$this->config->root_url.$url_path.$success.'&c='.$contract->id);
						else
							header('Location: '.$this->config->root_url.$url_path.$success);
						exit;
					}
				}
			}
		...
		if(isset($_SESSION['user_id']))
		{
			...
			if(!empty($salesflow))
			{
				...
				if(!empty($booking))
				{
					...
					$interval = date_diff(date_create($booking->depart), date_create($booking->arrive));
			...
			if(!empty($salesflow->application_data) || !empty($salesflow->additional_files))
			{
				...
				if(!empty($salesflow)) {
                    $ra_invoice_filter['booking_id'] = $salesflow->booking_id;
                }
				elseif(!empty($user->booking)) {
				...
				if($salesflow->type == 1 && $house->type == 1 && $booking->interval <= 30)
				{
					$tax_filter = [];
					$tax_filter['user_id'] = $user->id;
					if(!empty($user->booking))
						$tax_filter['booking_id'] = $booking->id;
					$tax_filter['type'] = 11; // Tax fee 
					$tax_filter['limit'] = 1;
					$tax_filter['count'] = 1;
					
					$tax_invoice = $this->orders->get_orders($tax_filter);
					if(!empty($tax_invoice))
					{
				    	$this->design->assign('tax_invoice', $tax_invoice);
					}
				}
			}
			...
			if($user_type == 'guarantor')
			{
                ...
                elseif (!empty($salesflow->additional_files) && $this->request->get('add_docs') != 'sended') {
                    header('Location: '.$this->config->root_url.'/guarantor/application?success=sended&add_docs=sended');
                    exit;
                }
			...
			if($this->request->files('dropped_files'))
			{
				...
				if(!empty($add_files))
				{
					...
					if(!empty($contract))
						header('Location: '.$this->config->root_url.$url_path.'?add_docs=sended&c='.$contract->id);
					else
						header('Location: '.$this->config->root_url.$url_path.'?add_docs=sended');
				}
			}
			elseif($this->request->method('post'))
			{
                ...
                if ($this->request->post('us_citizen'))
                ...
                $s->application_data['live_parents'] = trim($this->request->post('live_parents'));
  	  ...
  	  //   		if(!empty($u->files))
                ...
                if ((empty($contract) || $user_type == 'guarantor') && $this->config->host != 'dev') {
                    if (!empty($salesflow->application_data['social_number']) && empty($salesflow->transunion_id) && (($house->type == 1 && $booking->interval > 30) || $house->type != 1)) {
                        $r = $this->transunion->create_application($user->id);
                    }
                }
				if (empty($contract) && $this->config->host != 'dev') {
					/*
				...
				if (!empty($contract)) {
					...
					if($interval->days >= 28 && ($booking->sp_type == 0 || is_null($booking->sp_type)) && $booking->client_type_id != 2)
					{
			    		...
			    		if(!empty($invoicefee))
			    		{
			    			...
			    			// Philadelphia - The Mason on Chestnut
			    			if($house->id == 349) {
                                $application_invoice_price = $this->contracts->phila_application_fee;
                            }
							elseif ($house->id == 368) {
								$application_invoice_price = $this->contracts->application_fee_temple_house;
							}
			    			else {
                                $application_invoice_price = $this->contracts->application_fee;
                            }
			    			...
			    			$this->orders->add_purchase(array(
			    				...
			    				'price' => $application_invoice_price
			    			));
					...
					if($contract->outpost_deposit == 1 && $contract->price_deposit > 0 && empty($outpost_deposit) && $booking->client_type_id != 2)
			    	{
			    ...
			    // 			$this->users->update_user($user->id, array(
                    ...
                    if ((!empty($salesflow->application_data['social_number']) && empty($salesflow->transunion_id) && (($house->type == 1 && $booking->interval > 30) || $house->type != 1)) && $this->config->host != 'dev') {
                        $r = $this->transunion->create_application($user->id);
                    }
		...
		if(!empty($booking))
		{
            ...
            if (empty($contract)) {
                $contract = $this->contracts->get_contracts([
                    'reserv_id' => $booking->id,
                    'limit' => 1
                ]);
		...
		// hotel salesflow
		if($salesflow->type == 3 || $house->type == 1)
