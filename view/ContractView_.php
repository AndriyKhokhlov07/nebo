 ...
 //    }
	...
	function fetch()
	{
		...
		if(!empty($contract_info))
		{
			...
			{
				$this->salesflows->check_salesflow_status(null);
			}
						...
						if($reverv->apartment_id != 0)
						{
							...
							if(($contract_info->type >= 5 || $contract_info->type == 1) && ($house->id != 334 && $house->id != 337))
							{
								...
								$now_bjs = $this->beds->get_bookings(array(
									...
									'type' => 1, 
									'now' => 1, 
									'date_start_from' => $date_from_start->format('Y-m-d'),
									'not_canceled' => 1,
									'select_users' => true
								));
         ...
         //                        		$utility_price_val = 75;
						 		...
						 		$utility_price = $this->beds->get_utility_price($house->id, $reverv->type, $inv->date_from, $inv->date_to, $apartment);
