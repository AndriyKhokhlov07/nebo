...
class LandlordRentRollWView extends View
{
	private $params;
	private $table;
	...
	function fetch()
	{
					...
					*/
					...
					foreach($apartments as $a_id=>$a)
					{
						if(isset($a->a_bookings))
						{
							...
							foreach($a->a_bookings as $b_id=>$b)
							{
								if($b->rr_status == 'Vacant-Rented (p)' && count($apartments[$a_id]->a_bookings) > 1)
								{
									unset($apartments[$a_id]->a_bookings[$b_id]);
								}
							}
						...
						if(isset($a->b_bookings))
						{
							foreach($a->b_bookings as $object_id=>$b_objects)
							{
								if(!empty($b_objects->bookings))
								{
									...
									foreach($b_objects->bookings as $b_id=>$b)
									{
										if($b->rr_status == 'Vacant-Rented (p)' && count($apartments[$a_id]->b_bookings[$object_id]->bookings) > 1)
										{
											unset($apartments[$a_id]->b_bookings[$object_id]->bookings[$b_id]);
										}
									}
									...
									foreach($b_objects->bookings as $b_id=>$b)
									{
										...
										elseif($b->rr_status == 'Vacant-Rented' && $apartments[$a_id]->b_total[$object_id]->status > 1)
										{
											$apartments[$a_id]->b_total[$object_id]->status = 2;
											$apartments[$a_id]->b_total[$object_id]->price = $a->rooms[$a->beds[$object_id]->room_id]->price1;
										}
										elseif($b->rr_status == 'Vacant-Rented (p)' && $apartments[$a_id]->b_total[$object_id]->status > 2)
										{
											$apartments[$a_id]->b_total[$object_id]->status = 2;
											$apartments[$a_id]->b_total[$object_id]->price = $a->rooms[$a->beds[$object_id]->room_id]->price1;
										}
					...
					// ksort($bookings_other_period);
					// $this->design->assign('bookings_other_period', $bookings_other_period);

					// echo $this->params->now_month_last_day; exit;
		...
		return $this->design->fetch('landlord/rentroll_w.tpl');
	}
}
