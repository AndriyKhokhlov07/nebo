...
class LandlordRentRoll33View extends View
{
	...
	private function sheet_feature_booking($sheet, $n_row, $booking, $prev_booking)
	{
		...
		$sheet->setCellValue('M'.$n_row, $booking->days_count);
		$sheet->setCellValue('N'.$n_row, number_format($booking->leased_rent, 2, ',', ''), PHPExcel_Cell_DataType::TYPE_STRING);
		$sheet->setCellValue('O'.$n_row, number_format($booking->total_price_result, 2, ',', ''), PHPExcel_Cell_DataType::TYPE_STRING);


		$sheet->setCellValue('P'.$n_row, ($booking->client_type_id==5?'[House Leader] ':'').$booking->users_names);

		$sheet->setCellValue('Q'.$n_row, date('m/d/Y', $booking->u_arrive));
		$sheet->setCellValue('R'.$n_row, date('m/d/Y', $booking->u_depart));
	}
	...
	private function generate_xls($apartments)
	{
		...
		$sheet->getStyle("A1:R5")->applyFromArray([
			'fill' => [
		...
		$sheet->getStyle('A7:R7')->getAlignment()->setWrapText(true);
		$sheet->getStyle('A7:R7')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		...
		// align center
		$sheet->getStyle('A6:R'.$n_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		// align left
		$sheet->getStyle('J8:J'.$n_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$sheet->getStyle('P8:P'.$n_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		...
		// borders
		$sheet->getStyle('A6:R'.$n_row)->applyFromArray([
			'borders' => [
				'outline' => [
					'style' => PHPExcel_Style_Border::BORDER_THIN
				],
		...
		$sheet->getStyle('M6:M'.$n_row)->applyFromArray([
			'borders' => [
				'left' => [
					'style' => PHPExcel_Style_Border::BORDER_THIN,
					'color' => ['rgb' => '000000']
	...
	private function moth_calc($params = [])
	{
		...
		if(($u_arrive <= $u_first_day && $u_depart >= $u_first_day) || ($u_arrive <= $u_last_day && $u_depart >= $u_first_day))
		{
			...
			if(in_array(334, (array)$this->selected_house->id))
	 		...
	 		// 307 - Lafayette
	 		if(in_array(307, (array)$this->selected_house->id) && $b->apartment_id == 87)
	 		{
	 			$result->utility_price = 0;
	 		}
			*/

	 		if(isset($params['apartment']) && $params['apartment']->utility == 0)
	 		{
	 			$result->utility_price = 0;
	 		}
			...
			if(!empty($this->user->houses_ids))
			{
				...
				if(!empty($this->selected_house))
				{
					...
					*/
					...
					if($strtotime_now < strtotime('2021-07-01'))
						$strtotime_now = strtotime('now');					
						...
						foreach($bookings as $b)
						{
							...
							*/
							...
							$b->month1 = $this->moth_calc([
								...
								'booking' => $b,
								'apartment' => $apartments[$b->apartment_id]
							]);
