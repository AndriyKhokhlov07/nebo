...
class LandlordRentRoll32View extends View
{
	private $params;
	private $table;
	...
	function fetch()
	{
					...
					*/
					...
					foreach($apartments as $a_id=>$a)
					{
						if(isset($a->a_bookings))
						{
							...
							foreach($a->a_bookings as $b_id=>$b)
							{
								if($b->rr_status == 'Alumni' && count($apartments[$a_id]->a_bookings) > 1)
								{
									// unset($apartments[$a_id]->a_bookings[$b_id]);
								}
							}
							...
							foreach($a->b_bookings as $object_id=>$b_objects)
							{
								if(!empty($b_objects->bookings))
								{
									...
									foreach($b_objects->bookings as $b_id=>$b)
									{
										...
										elseif($b->rr_status == 'Alumni' && $apartments[$a_id]->b_total[$object_id]->status > 2)
										{
											$apartments[$a_id]->b_total[$object_id]->status = 2;
											$apartments[$a_id]->b_total[$object_id]->price = $a->rooms[$a->beds[$object_id]->room_id]->price1;
										}
					...
					if(!empty($feature_bookings))
					{
						foreach($feature_bookings as $b)
						{
							...
							if($b->type == 1) // beds
							{
								...
								$feature_beds_bookings[$b->object_id][$b->u_arrive.$b->id] = $b;
							}
							elseif($b->type == 2) // apartments
							{
								$feature_apartments_bookings[$b->object_id][$b->u_arrive.$b->id] = $b;
							}
						}
		...
		return $this->design->fetch('landlord/rentroll_32.tpl');
	}
}
