...
class RestockingView extends View
{
	...
	public function fetch()
	{
		...
		$this->restocking_type = $this->request->get('type', 'integer');
	...
	// Select House
	// fetch_select_house($url)
	private function fetch_select_house()
	{
		// Houses
		$houses_ids = array();
		...
		foreach ($houseleaders_houses as $hl) 
		{
			$houses_ids[] = $hl->house_id;
		}
		foreach ($housecleaners_houses as $hc) 
		{
			$houses_ids[] = $hc->house_id;
		}
		if(!empty($houses_ids))
			$houses = $this->pages->get_pages(array('id'=>$houses_ids, 'menu_id'=>5, 'visible'=>1, 'not_tree'=>1));

		foreach ($houses as $h) 
		{
			if(empty($houses[$h->parent_id]))
				$houses[$h->parent_id] = $this->pages->get_page(intval($h->parent_id));
		}
