...
require_once(realpath('./api/Config.php'));
...
abstract class _Model implements ModelInterface
{
    use Schema;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->fields = self::getSchema()['tables'][$this->getTable()];
    ...
    public static function find($pk): ?self
    {
        ...
        $list = self::getList(['*'], ["`$fieldName` = $pk"], [], 1);
        if(!empty($list)){
            return $list[0];
        }
        return null;
    ...
    public static function getList(?array $select = ['*'], ?array $conditions = [], ?array $orders = [], ?int $limit = null, int $offset = null)
    {
...
//        $result = self::getCollection($list);
//        $result = null;
    ...
    public function __get($name)
    {
        ...
        }elseif (isset($this->cacheValues[$name])){
            return $this->cacheValues[$name];
        }elseif (method_exists($this, $name)){
            $this->cacheValues[$name] = $this->{$name}();
            return $this->cacheValues[$name];
        }else{
            return null;
        }
...
//                $item->joined[]
    ...
    private function queryBuilderGetList(array $select = ['*'], array $conditions = [], array $orders = [], int $limit = 0, int $offset = 0): string
    {
