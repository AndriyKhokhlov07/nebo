...
trait Schema
{
    ...
    private static function getConfig()
    {
        if(empty(self::$config)){
            self::$config = \Backend::backendApp()->config->getConfig();
        }
        return self::$config;
    }
    ...
    private static function createMigrationModelIfNotExists(): void
    {
        if(self::isTableExits(self::getDBMigrationTable())){
            return;
        }
    ...
    protected static function getQuery($query = null)
    {
        if($query === null){
            $query = self::queryBuilder();
        }

        $sql = ($query instanceof QueryBuilder) ? $query->getSql() : $query;
    ...
    protected static function getQueryObjects($query = null)
    {
        if($result = self::getQuery($query)){
            $data = [];
    ...
    protected static function getQueryAssocs($query = null)
    {
        if($result = self::getQuery($query)){
            $data = [];
            while ($row = $result->fetch_assoc()){
