...
{	
	public function fetch()
	{
		...
		// need_guarantor upd
		$need_guarantor = $this->request->get('need_guarantor', 'integer'); 
		if(!empty($need_guarantor))
		{
			$salesflow_id = $need_guarantor;
			...
			if(!empty($user_id))
			{
				...
				if(!empty($user) && !empty($salesflow))
				{
					...
					// $email_template = $this->design->fetch($this->config->root_dir.'design/'.$this->settings->theme.'/html/email/need_guarantor.tpl');
					// echo $email_template; exit;


					// update salesflow
					// 4 - need a guarantor
					$this->salesflows->update_salesflow($salesflow->id, ['status' => 4]);
			...
			header('Location: '.$this->config->root_url.'/backend/?module=UserAdmin&id='.$user->id);
			exit;
		}
		// need_guarantor upd (end)

		// dont extend upd
		$dont_extend = $this->request->post('dont_extend', 'integer'); 
		if(!empty($dont_extend))
		{
			$user_id = $this->request->get('id', 'integer');
			...
			if(!empty($user_id))
			{
				...
				if(!empty($user))
				{
					if($dont_extend == 1)
						$subtype = 11;
					elseif($dont_extend == 2)
					{
						$dont_extend = 0;
						$subtype = 12;
					}

					$this->users->update_user($user->id, ['dont_extend'=>$dont_extend]);

					$log_value = $this->request->post('dont_extend_note');

		            $extend_log = new stdClass();
					$extend_log->parent_id = $user->id;
					$extend_log->type = 2;
					$extend_log->subtype = $subtype;
					$extend_log->sender_type = 2;
					$extend_log->sender = $manager->login;
					$extend_log->user_id = $user->id;
					if(!empty($log_value))
						$extend_log->value = $log_value;

					// add log
					$this->logs->add_log($extend_log);
				}
			}
			header('Location: '.$this->config->root_url.'/backend/?module=UserAdmin&id='.$user->id);
			exit;
		...
		// Cassa hotel upd
		$send_confirmation = $this->request->get('send_confirmation', 'integer'); 
		if(!empty($send_confirmation))
		{
			$salesflow_id = $send_confirmation;
			$salesflow = $this->salesflows->get_salesflows([
				'id' => $salesflow_id,
				'limit' => 1
			]);
			...
			if(!empty($user_id))
			{
				...
				if(!empty($user) && !empty($salesflow))
				{
					...
					$this->salesflows->update_salesflow($salesflow->id, ['approve' => 1]);
					...
					if(!empty($booking))
					{
						...
						if(!empty($house))
						{
							...
							$first_month_invoice = current($this->orders->get_orders(['booking_id'=>$booking->id, 'deposit'=>0, 'limit' => 1, 'sort' => 1]));
							...
							if(!empty($user->email))
							{
								...
								$this->design->assign('salesflow', $salesflow);
								$this->design->assign('user', $user);
								if(!empty($first_month_invoice))
									$this->design->assign('first_month_invoice', $first_month_invoice);

								$email_template = $this->design->fetch($this->config->root_dir.'design/'.$this->settings->theme.'/html/email/booking_confirmation_email.tpl');
								$subject = $this->design->get_var('subject');
								$this->notify->email($user->email, $subject, $email_template, $this->settings->notify_from_email);
								...
								$log_params = [
					                ...
					                'sender_type' => 2,
					                'sender' => $manager->login,
					                'value' => "Confirmation email sent to guest."
								];
		...
		if(!empty($_POST['to_status']) || !empty($approve))
		{
			...
			if(!empty($approve))
			{
				...
				$salesflow_id = $this->request->get('salesflow', 'integer'); 
	            ...
	            $this->salesflows->approve_salesflow(array('salesflow_id'=>$salesflow_id, 'add_to_contract'=>$add_to_contract, 'sender'=>$manager->login, 'sender_type'=>2));
			}
			...
			if(!empty($user->id) && empty($approve))
		...
		if(!empty($_POST['user_info']) && empty($_POST['add_log']))
		{
			...
			$user = current($this->users->get_users(['id'=>intval($user->id), 'salesflow_info'=>1]));
		...
		if(!empty($id))
		{
			$user = current($this->users->get_users(['id'=>intval($id), 'salesflow_info'=>1]));
		...
		if(!empty($user->id))
		{
			...
			/*
			if($user->status == 10) // Guarantor
			{
				$application_file_path = $this->config->users_files_dir.$user->id.'/application.pdf';
				if(file_exists($application_file_path))
				{
					$f_size = filesize($application_file_path);
					$file_size = $f_size.' B';
					if($f_size > 999)
						$file_size = round(($f_size / 1024), 2).' KB';
					if($f_size > 999000)
						$file_size = round(($f_size / 1024 / 1024), 2).' MB';
					
					$user->docs_files['application'] = (object)[
						'name' => 'Application Doc',
						'url' => $application_file_path,
						'ext' => 'pdf',
						'date' => filectime($application_file_path),
						'size' => $file_size
					];
				}

				$guarantor_agreement_file_path = $this->config->users_files_dir.$user->id.'/guarantor_agreement.pdf';
				if(file_exists($guarantor_agreement_file_path))
				{
					$f_size = filesize($guarantor_agreement_file_path);
					$file_size = $f_size.' B';
					if($f_size > 999)
						$file_size = round(($f_size / 1024), 2).' KB';
					if($f_size > 999000)
					...
					$user->docs_files['guarantor_agreement'] = (object)[
						'name' => 'Guarantor Agreement Doc',
						'url' => $guarantor_agreement_file_path,
						'ext' => 'pdf',
						'date' => filectime($guarantor_agreement_file_path),
						'size' => $file_size
					];
				}
			}
			*/
			...
			if(!empty($bookings_))
			{
				...
				foreach($bookings_ as $k=>$bj)
				{
					...
					$bj->days_count = round($u_bj_interval / (24 * 60 * 60) + 1);


					// Airbnb
					if(in_array($bj->client_type_id, [2]))
					{

						unset($b->price_month);

						$bj->calculate->price_30_days = round($bj->price_day * 30);
						$bj->calculate->total = round($bj->price_day * $bj->days_count);
					}
			...
			$orders_ = $this->orders->get_orders(array('user_id'=>$user->id, 'sort_date_from'=>1, 'not_status'=>3));
			...
			$contracts_ = $this->contracts->get_contracts(array('user_id'=>$user->id));
		...
		if($this->request->post('notification'))
		{
			...
			elseif($notification == 'bgcheck')
			{
				...
				elseif(empty($user->email))
					$this->design->assign('message_error', 'Enter Email');
				elseif(empty($user->active_salesflow_id) || !isset($user->active_salesflow_id))
					$this->design->assign('message_error', 'Create booking and salesflow first');
				else
				{
		...
		if(!empty($user->id))
		{
			...
			if(!empty($logs))
			{
				...
				foreach($logs as $log)
				{
					...
					elseif(in_array($log->type, [2, 13, 14, 15, 16]))
					{
						$user_logs_[$log->id] = $log;
						// airbnb/hotel salesflow sent
						if($log->subtype == 9)
							$hotel_salesflow_logs[$log->parent_id] = $log;
						elseif($log->subtype == 10)
							$airbnb_salesflow_logs[$log->parent_id] = $log;
					}
					elseif($log->type == 7)
					...
					if($log->type == 15) // Need a Guarantor
					{
						$user->isset_guarantor = true;
					}


					$logs_tr[$log->type][$log->subtype][$log->id] = $log;
				}
			}

			// print_r($logs); exit;
			$this->design->assign('hotel_salesflow_logs', $hotel_salesflow_logs);
			$this->design->assign('airbnb_salesflow_logs', $airbnb_salesflow_logs);
			$this->design->assign('logs', $user_logs_);
			$this->design->assign('logs_tr', $logs_tr);
			...
			if(!empty($salesflows))
			{
				...
				foreach ($salesflows as $s) 
				{
					...
					{
						if($c->reserv_id == $s->booking_id && $c->status == 1)
							$s->contract = $c;
						elseif($c->reserv_id == $s->booking_id)
							$s->contracts[$c->id] = $c;
					}
		...
		$create_salesflow = $this->request->get('create_salesflow');
		if(!empty($create_salesflow))
		{
			$booking = $this->beds->get_bookings(['id'=>intval($create_salesflow), 'limit'=>1]);

			if(!empty($booking))
			{
				$saleflow = new stdClass();
		...
		$start_airbnb_salesflow = $this->request->get('start_airbnb_salesflow');
		if(!empty($start_airbnb_salesflow))
		{
			$salesflow = $this->salesflows->get_salesflows(['id'=>$user->active_salesflow_id, 'limit'=>1]);

			if($salesflow->type == 1 || $salesflow->type == 3)
			{
				$this->contracts->start_first_salesflow(0, $salesflow->user_id, array('salesflow_id'=>$salesflow->id, 'sender'=>$manager->login, 'sender_type'=>2));
			}

			header('Location: '.$this->config->root_url.'/backend/?module=UserAdmin&id='.$user->id);
		}

		$start_hotel_salesflow = $this->request->get('start_hotel_salesflow');
		if(!empty($start_hotel_salesflow))
		{
			$salesflow = $this->salesflows->get_salesflows(['id'=>$user->active_salesflow_id, 'limit'=>1]);

			if($salesflow->type == 1 || $salesflow->type == 3)
			{
				$this->salesflows->start_hotel_salesflow($salesflow, $salesflow->user_id, array('sender'=>$manager->login, 'sender_type'=>2));
			}
		...
		if(!empty($user->isset_guarantor))
		{
			...
			if(!empty($uu))
			{
				$uu_ids = $this->request->array_to_key($uu, 'child_id');
				$user->guarantors = $this->users->get_users([
					'id' => array_keys($uu_ids)
				]);
			}
		}
		// Guarantor (end)
		...
		if(!is_array($user->files) && !empty($user->files))
			$user->files = unserialize($user->files);
		if(!is_array($user->blocks) && !empty($user->blocks))
			$user->blocks = unserialize($user->blocks);
