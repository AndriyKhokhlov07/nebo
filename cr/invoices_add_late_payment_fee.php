    ...
    function fetch()
    {
        ...
        do{
            ...
            $query = $this->db->placehold("SELECT 
						...
						LEFT JOIN s_logs AS log_info ON log_info.parent_id=o.id AND log_info.type=3 AND log_info.subtype IN (16, 17)
                        LEFT JOIN s_logs AS log_add ON log_add.parent_id=o.id AND log_add.type=3 AND log_add.subtype IN (18, 19)
						WHERE 1
						    $where
                        GROUP BY o.id
            ...
            if (!empty($orders)) {
                ...
                foreach ($orders as $order){
                    ...
                    if (!isset($order->hasLatePaymentFee)) {

                        $this->orders->add_purchase([
                            'order_id' => $order->id,
                            'type' => 8,
                            'variant_id' => 0,
                            'product_name' => 'Late payment Fee',
                            'price' => ($order->booking_type == 2) ? $this->contracts->latePaymentFeeApertments : $this->contracts->latePaymentFee
                        ]);

                        $this->logs->add_log([
                            'parent_id'   => $order->id,
                            'type'        => 3,  // Invoice
                            'subtype'     => 14, // Add Late Payment Fee
                        ...
                        if (!empty($order->users)) {
                                ...
                                if (
                                    ...
                                    foreach ($user->payment_methods_details as $pm) {
                                        if ($pm->response->Success == 1 && in_array($pm->payee_id, $housesPayeeIds[$order->house_id])) {
                                            ...
                                            $email_template = $this->design->fetch($this->config->root_dir . 'design/' . $this->settings->theme . '/html/email/invoice/autocharge_added_late_payment_fee.tpl');
                                            ...
                                            $this->logs->add_log([
                                                'parent_id'   => $order->id,
                                                'type'        => 3,  // Invoice
                                                'subtype'     => 19, // Sent notice about Added Late Payment Fee (Autocharge)
                                                'sender_type' => 1,  // System
                                            ]);
                            ...
                            if (!$order->autocharge) {
                                foreach($order->users as $user) {
                                    ...
                                    $email_template = $this->design->fetch($this->config->root_dir . 'design/' . $this->settings->theme . '/html/email/invoice/added_late_payment_fee.tpl');
                                    ...
                                    $this->logs->add_log([
                                        'parent_id'   => $order->id,
                                        'type'        => 3,  // Invoice
                                        'subtype'     => 18, // Sent notice about Added Late Payment Fee (Autocharge)
                                        'sender_type' => 1,  // System
                                    ]);
