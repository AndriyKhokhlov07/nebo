    ...
    function fetch()
    {
        ...
        do{
            ...
            $query = $this->db->placehold("SELECT 
						...
						LEFT JOIN s_logs AS log ON log.parent_id=o.id AND log.type=3 AND log.subtype IN (14, 15)
						WHERE 1
						    $where
                        GROUP BY o.id
                                ...
                                if (
                                    ...
                                    foreach ($user->payment_methods_details as $pm) {
                                        if ($pm->response->Success == 1 && in_array($pm->payee_id, $housesPayeeIds[$order->house_id])) {
                                            ...
                                            $email_template = $this->design->fetch($this->config->root_dir . 'design/' . $this->settings->theme . '/html/email/invoice/autocharge_late_payment_fee_info.tpl');
                                            ...
                                            $this->logs->add_log([
                                                'parent_id'   => $order->id,
                                                'type'        => 3,  // Invoice
                                                'subtype'     => 15, // Sent notice about don't pay (Autocharge)
