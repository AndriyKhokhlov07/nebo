...
class Test extends Backend
{
	function fetch()
	{
        ...
        $query = $this->db->placehold("SELECT 
                ...
                label.label_id as label
            FROM __orders AS o
            LEFT JOIN __contracts c ON c.id = o.contract_id
            LEFT JOIN __bookings book ON book.id = o.booking_id
            LEFT JOIN __orders_labels label ON label.order_id = o.id AND label.label_id = 13
            WHERE 
                o.payment_method_id=12
                AND o.date>='2021-09-01'
            GROUP BY o.id 
            ORDER BY o.date
        ");
        ...
        if(!empty($invoices)) {
            ...
            header("Content-Disposition: attachment; filename=invoices_paid_by_deposit.csv"); 
