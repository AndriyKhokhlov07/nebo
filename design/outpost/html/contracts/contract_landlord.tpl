...
<p><strong>Landlord:</strong> {$landlord->name}</p> <!-- company -->
...
<ol type="a">
	...
	<li><strong style="font-weight: 800;">Tenants consent to the assignment of this Lease by any other tenant in the Apartment to an assignee of Landlord’s choosing. Tenants knowingly waives any right to know the identity of such assignee in advance of the assignment. Tenants agree to be jointly and severally liable for all obligations of the Lease with such assignee.</strong> Landlord and Tenants agree that: (1) Landlord covenants to find Tenants for this Lease and introduce them to each other; (2) Tenants are not responsible for finding Tenants.</li>
...
<ol type="a">
	...
	<li>Any rent payments are final and non refundable. Attempted cancellations through credit card processor, chargebacks for non-fraudulent transactions through the Payment System (Stripe/Paypal/via Credit/Debit Card/ACH) will be subject to investigation and these individuals will be prosecuted to the fullest extent of the law.</li>
</ol>
...
<ol type="a">
	...
	<li>Tenant agrees that Outpost may share personal data including but not limited to Name/Surname, email, telephone number, current address and zip code with HelloRented.</li>
</ol>

<h2>9. CONTENTS OF APARTMENT PROVIDED BY LANDLORD</h2>

<p>The Apartment is leased with the equipment, kitchen appliances and furniture in good and working condition. Tenants agree to return all items provided at the start of Term in as good condition as when received, reasonable wear and tear excepted. Tenants will be responsible for all breakage or other damage to items provided and such damages are deductible from the Security Deposit.</p>
...
<ol type="a">
	...
	<li>Landlord will provide cold and hot water and heat as required by law, elevator service if the Building has elevator equipment. Landlord will open accounts and pay charges for electricity, gas as well as internet services, provided, however, Landlord reserves the right to bill Tenant for any amounts exceeding $50 per Tenant per month in the aggregate and in connection therewith. Tenants are not entitled to any reduction in the Monthly Rent because of a stoppage or reduction of any of the above services unless such reduction is required by law. No other utilities or services are to be furnished by Landlord or Landlord’s agent or used by Tenants in the Apartment without prior written consent of Landlord or Landlord’s Agent and on the terms and conditions specified in such written consent. Tenants shall make no alterations to the Apartment.</li>
...
{foreach  $users as $u}
{if $contract_user->id != $u->id}
<p>Tenant {$u->name}</p>
{if $u->contract}
<p>Signature</p>
<img src="{$u->contract->signature}" alt="{$u->name}" width="120">
<br>
<p>Date {$u->contract->date_signing|date}</p>
{else}
<p>Date {$u->inquiry_arrive|date}</p>
{/if}
<br>
<br>
{/if}
...
<div class="fx ch2">
	<div>
		<p>Tenant {$u->name}</p>
		{if $u->contract}
		<p>Individual Term {$u->contract->date_from|date} - {$u->contract->date_to|date}</p>
		{*<p>Pro Rata Share of Monthly Rent {$u->contract->price_month}</p>*}
		<p>Date {$u->contract->date_signing|date}</p>
		{else}
		<p>Individual Term {$u->inquiry_arrive|date} - {$u->inquiry_depart|date}</p>
		{*
		<p>Pro Rata Share of Monthly Rent {$u->contract->price_month}</p>
		*}
		<p>Date {$u->contract->date_signing|date}</p>
...
{if $contract_info->invoices|count > 1}
...
<p>Outpost provides House Rules to assure a safe and productive environment. If a Tenant breaks any of the House Rules, Outpost reserves the right not to renew or extend the lease.</p>

<p>These House Rules are incorporated into the Lease. By adopting these House Rules, Tenant understands that Tenant is obligated to follow the rules and any violations thereof are subject to the penalties and remedies set forth herein or in any of the incorporated agreements.</p>

<h4>Move-in</h4>
<ul>
	<li>When Tenant moves into the premises, Tenant will be required to furnish a valid government photo ID card and update all contact information for the Property Manager.</li>
...
<h4>Guests</h4>
<ul>
	<li>Unless otherwise permitted in writing pursuant to this Agreement, Tenants are not permitted overnight guests on the premises, including on a common couches, in hallways or in any common areas.</li>
	<li>Tenants are responsible for the appropriate behavior of their guests. Should Tenants’s guest(s) not behave responsibly and appropriately in accordance with the same rules and policy which Tenants is obligated to abide by, they will be asked to leave immediately.</li> 
...
<h4>Cleanliness</h4>
<ul>
	<li>In common areas and common kitchens Tenant will wash all kitchen appliances it uses each time after using them and will clean the sink of any food remnants it caused to be there. </li>
	<li>Tenant will leave no dishes or kitchen equipment in the sink of the common kitchens or common areas. Repeated violations of this rule can lead to the termination of Tenant’s Lease. Owner/Landlord and/or Property Manager reserves the right to check the cameras to identify who is violating this rule. Owner/Landlord and/or Property Manager only checks cameras once other tenants or House Leaders notify us of a violation.</li>
	<li>Tenants will have the responsibility of taking out the trash, wiping down the countertops, and emptying/running the dishwasher.</li>
	...
	<li>Tenant will not leave their belongings in the common areas of the Property.  Any such mislaid belongings may be removed and placed in a Lost and Found box.</li>
	<li>Tenant will clean the bathrooms in common areas of the Premises after they use the bathroom. Tenant will make sure to leave the bathrooms in the condition it was found after Property Manager’s housekeeper has cleaned it.</li>
	...
	<li>Tenants are required to keep the common areas clean, sanitary, and organized.</li>
	<li>Property Manager shall provide cleaning services on a bi-weekly basis for all common areas and bathrooms. During those cleaning days, Tenants should organize their personal belongings in a way that allows the cleaners to mop and sweep the floor. The service schedule/frequency may be amended according to governmental restrictions or to provide safety for tenants and/or employees of the company.</li>
...
<ul>
	...
	<li>Tenants consent to being photographed and/or videotaped in public areas and during Property Manager-sponsored events. These photographs and videos can be used in Outpost or its affiliates social media accounts, websites and blogs, newsletters, media content.</li>
	...
	<li>Tenants are expressly forbidden from placing locks on any bedroom, bathroom or other doors.</li>
</ul>
