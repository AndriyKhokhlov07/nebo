...
{/if}
<p>For good and valuable consideration, the receipt of which is hereby acknowledged, this Lease Amendment Number {$contract_info->id}, dated this <span class="border_b">{$contract_info->date_created|date}</span> by and between Landlord and Tenants to be incorporated into a certain Lease Agreement for {$apartment->name} at <span class="border_b">{$contract_info->rental_name|escape}</span> dated <span class="border_b">{if $first_contract}{$first_contract->date_signing|date}{else}{$contract_info->date_created|date} {/if}</span> between Landlord and Tenants, is hereby incorporated into said document. All terms and conditions of the original lease, except revisions listed below, shall remain in full force.</p>
...
<p><strong>Building:</strong> {$house->blocks2['address']}</p>
