...
{elseif $booking->client_type_id==2 && (($contract && $contract->interval < 30) || (!$contract && $booking->interval < 30))}
...
{/if}
	...
	{elseif $salesflow->type == 3 && $deposit_invoice}
    	{$next_step_link = "order/{$deposit_invoice->url}?s={$salesflow->id}&u={$user->id}"}
	{elseif $invoicefee}
		{$steps_count = 3}
	...
	{if $smarty.get.success=='sended' && $booking->client_type_id != 2 && ($contract->interval >= 28 || ($salesflow->application_data['employment_status'] == 2 && $booking->client_type_id != 2 &&  $salesflow->application_data['us_citizen'] != 1)) && $smarty.get.add_docs!='sended'}
	<h1 class="bold_h1_new">Additional documents</h1>
	<div class="txt">
		<p>In order to verify your employment, we are requesting:</p>
		<ul>
			{if $salesflow->application_data['employment_status'] == 2}
				<li>I-20 form from your university</li>
				<li>Student visa (J1, M1, F1)</li>
				<li>Passport page (needs to be your internationally recognized passport)</li>
			{else}
	...
	{elseif ($smarty.get.success=='sended' && ((($booking->client_type_id != 2 && $contract->interval < 28) || $booking->client_type_id == 2) || ($user->us_citizen == 2 && $contract->interval < 90) )) || $smarty.get.add_docs=='sended'}
				<div class="info">
					<h1 class="bold_h1_new center">Thank You!</h1>
					<p class="center">
						{* Airbnb *}
						{*
						{if $user->booking->client_type_id == 2}
							We will send your an email and ask to sign up a lease agreement and a COVID waiver.
						{/if}
						*}
					</p>
				</div>
			...
			<div class="txt">
				...
				{elseif $user_type=='guarantor'}
					...
					{$next_step_link="{$config->root_url}/guarantor/agreement"}
					{include file='guarantor/bx/steps_apps.tpl'}
				{elseif $salesflow->type == 1 && !$covid_form}
					{$active_step=1}
					{$steps_type = 'airbnb'}
					{include file='bx/steps_apps.tpl'}
					<br>
	...
	<div class="user_check hl_checklist">
		<form method="post" name="rental_application" enctype="multipart/form-data">
			...
			<div class="check_wrapper">
				...
				<input class="radio_h" id="student_y" type="radio" name="student" required value="1"{if $salesflow->application_data['employment_status']==2} checked{/if}>
				<input class="radio_h" id="student_n" type="radio" name="student" required value="0"{if $salesflow->application_data['employment_status'] && $salesflow->application_data['employment_status']!=2} checked{/if}>

				{if ($contract->interval < 180 && $contract->reserv->type == 1) || !$contract}{else}
				<input class="radio_h" id="live_parents_y" type="radio" name="live_parents" required value="1"{if $salesflow->application_data['live_parents']==1} checked{/if}>
				<input class="radio_h" id="live_parents_n" type="radio" name="live_parents" required value="0"{if $salesflow->application_data['live_parents'] && $salesflow->application_data['live_parents']!=1} checked{/if}>
				{/if}
				...
				<div class="input_block">
					...
					<div class="fx ch3">
						...
						<div class="radio_h_bx">
							<label class="req">Are you a student?</label>

							<div class="radio_buttons">
								<label for="student_y">Yes</label>
								<label for="student_n">No</label>
							</div><!-- radio_buttons -->
						</div>
						{if ($contract->interval < 180 && $contract->reserv->type == 1) || !$contract}{else}
						<div class="radio_h_bx live_parents_h_bx">
							<label class="req">Do you live with parents?</label>

							<div class="radio_buttons">
								<label for="live_parents_y">Yes</label>
								<label for="live_parents_n">No</label>
							</div><!-- radio_buttons -->
						</div>
						{/if}
				...
				<div class="content_block1 visible v1">
					...
					<div class="padd_bx">
						...
						<div class="input_block">
							...
							<div class="fx ch2">
								...
								<div class="input_wrapper">
									<label for="itin" class="req">ITIN</label>
									<input class="inp_i{if $user->us_citizen==1} required{/if}" id="itin" type="text" name="itin" value="{$salesflow->application_data['itin']}" placeholder="987-65-4321" data-required>
								</div>
							</div><!-- fx -->
							<div class="fx ch2">
								<div class="input_wrapper">
							...
							<div class="fx ch2">
								<div class="input_wrapper social_number_bx">
									<label for="social_number"{if $user->us_citizen==1} class="req"{/if}>Social Security Number</label>
									<input class="inp_i{if $user->us_citizen==1} required{/if}" id="social_number" type="text" name="social_number" value="{$salesflow->application_data['social_number']}" placeholder="123-45-6789" data-required>
									<p class="info_txt{if $user->us_citizen==1} hide{/if}">*Fill this field ONLY if you have SSN</p>
								</div>
								<div class="input_wrapper">
						...
						<div class="input_block">
							<div class="fx ch2">
								<div class="fx ch2_m10 ch2_1_">
									...
									<div class="state_block">
										...
										<div class="input_wrapper state_bx2">
											...
											<input class="inp_i" id="state2" type="text" name="blocks[state]" value="{$salesflow->application_data['state']|escape}" placeholder="">
										</div>
								...
								<div class="input_wrapper">
									...
									<input class="inp_i required" id="city" type="text" name="city" value="{$salesflow->application_data['city']|escape}" placeholder="New York" data-required data-pattern="{literal}[A-Za-z0-9\s\.\-\(\)'`,]{3,}{/literal}">
							...
							<div class="fx ch2">
								<div class="fx ch2_1">
									<div class="input_wrapper">
										...
										<input class="inp_i google-search-address required" id="street_address" type="text" name="street_address" value="{$salesflow->application_data['street_address']|escape}" placeholder="" data-required data-pattern="{literal}[A-Za-z0-9\s\.\-\(\)'`,]{3,}{/literal}">
									</div>
							...
							{else}
							<div class="fx ch2">
								<div class="input_wrapper hide_for_live_parents_y">
								...
								<div class="fx ch2 ch1_2">
									<div class="input_wrapper">
										...
										<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="pets" type="text" name="blocks[pets]" value="{$salesflow->application_data['pets']|escape}" placeholder="" data-required>
									</div>

									<div class="input_wrapper hide_for_live_parents_y">
										<label {if $user->us_citizen!=2}class="req"{/if} for="pets_breeds">Pets – Specify breeds</label>
										<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="pets_breeds" type="text" name="blocks[pets_breeds]" value="{$salesflow->application_data['pets_breeds']|escape}" placeholder="" data-required>
									</div>
								</div><!-- fx -->
							</div><!-- fx -->

							<div class="fx ch2 hide_for_live_parents_y">
								<div class="fx ch2_m10">
									<div class="input_wrapper">
										<label {if $user->us_citizen!=2}class="req"{/if} for="reason_for_moving">Reason for moving</label>
										<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="reason_for_moving" type="text" name="blocks[reason_moving]" value="{$salesflow->application_data['reason_moving']|escape}" placeholder="" data-required>
									</div>
									<div class="input_wrapper">
							...
							<div class="fx ch3 hide_for_live_parents_y">

								<div class="input_wrapper">
									<label {if $user->us_citizen!=2}class="req"{/if} for="current_landlord">Current Landlord</label>
									<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="current_landlord" type="text" name="blocks[current_landlord]" value="{$salesflow->application_data['current_landlord']|escape}" placeholder="" data-required>
								</div>

								<div class="input_wrapper">
									<label {if $user->us_citizen!=2}class="req"{/if} for="current_landlord_address">Their Address</label>
									<input class="inp_i google-search-address{if $user->us_citizen!=2} required{/if}" id="current_landlord_address" type="text" name="blocks[cl_address]" value="{$salesflow->application_data['cl_address']|escape}" placeholder="" data-required>
								</div>

								<div class="input_wrapper">
									<label {if $user->us_citizen!=2}class="req"{/if} for="current_landlord_phone">Phone</label>
									<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="current_landlord_phone" type="text" name="blocks[cl_phone]" value="{$salesflow->application_data['cl_phone']|escape}" placeholder="" data-required>
								</div>

							</div><!-- fx / ch3 -->


							<hr class="hide_for_live_parents_y">

							<p class="title hide_for_live_parents_y">Previous Address</p>

							<div class="fx ch2 hide_for_live_parents_y">
								<div class="fx ch2_m10">
									<div class="input_wrapper">
										<label {if $user->us_citizen!=2}class="req"{/if} for="previous_state">State</label>
										<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="previous_state" type="text" name="blocks[prev_state]" value="{$salesflow->application_data['prev_state']|escape}" placeholder="" data-required>
									</div>
									<div class="input_wrapper">
										<label {if $user->us_citizen!=2}class="req"{/if} for="previous_city">City</label>
										<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="previous_city" type="text" name="blocks[prev_city]" value="{$salesflow->application_data['prev_city']|escape}" placeholder="" data-required>
									</div>
								</div>
								...
								<div class="fx ch2_1">
									<div class="input_wrapper">
										...
										<input class="inp_i google-search-address{if $user->us_citizen!=2} required{/if}" id="previous_address" type="text" name="blocks[prev_address]" value="{$salesflow->application_data['prev_address']|escape}" placeholder="" data-required>
									</div>
							...
							<div class="fx ch2 hide_for_live_parents_y">
								<div class="fx ch2_m10">
									<div class="input_wrapper">
										...
										<input class="inp_i input_zip{if $user->us_citizen!=2} required{/if}" id="previous_zip" type="text" name="blocks[prev_zip]" value="{$salesflow->application_data['prev_zip']|escape}" placeholder="" data-required>
									</div>
								</div><!-- fx -->
							</div><!-- fx -->
							...
							<div class="fx ch3 hide_for_live_parents_y">
								...
								<div class="input_wrapper">
									...
									<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="previous_landlord" type="text" name="blocks[prev_landlord]" value="{$salesflow->application_data['prev_landlord']|escape}" placeholder="" data-required>
								</div>

								<div class="input_wrapper">
									<label {if $user->us_citizen!=2}class="req"{/if} for="previous_landlord_address">Their Address</label>
									<input class="inp_i google-search-address{if $user->us_citizen!=2} required{/if}" id="previous_landlord_address" type="text" name="blocks[pl_address]" value="{$salesflow->application_data['pl_address']|escape}" placeholder="" data-required>
								</div>

								<div class="input_wrapper">
									<label {if $user->us_citizen!=2}class="req"{/if} for="previous_landlord_phone">Phone</label>
									<input class="inp_i{if $user->us_citizen!=2} required{/if}" id="previous_landlord_phone" type="text" name="blocks[pl_phone]" value="{$salesflow->application_data['pl_phone']|escape}" placeholder="" data-required>
								</div>
					...
					<div class="padd_bx hide_for_student">
						<div class="input_block">

							<div class="fx ch2">
								<div class="input_wrapper{if $salesflow->application_data['employment_status']==2} hide{/if}">
							...
							{if (($contract->interval < 180 && $contract->reserv->type == 1) || !$contract) && $user_type=='tenant'}
							{else}
							<div class="fx ch2">
								<div class="input_wrapper">
							...
							<div class="fx ch2">
								<div class="input_wrapper">
									...
									<input class="inp_i google-search-address{if $user->us_citizen!=2} required{/if}" id="work_address" type="text" name="blocks[work_address]" value="{$salesflow->application_data['work_address']|escape}" placeholder="" data-required>
								</div>
								<div class="input_wrapper">
					...
					<div class="input_block">
						<div class="fx ch3" style="align-items: flex-end;">
							...
							<div class="input_wrapper">
								...
								<div class="inp_i file_block">
									<div class="preview_block{if $salesflow->application_data['files'] && $salesflow->application_data['files']['passport']} active{/if}">
										<img {if $salesflow->application_data['files'] && $salesflow->application_data['files']['passport']}src="{$config->users_files_dir}{$user->id}/{$salesflow->application_data['files']['passport']}"{/if}>
										<i class="delete fa fa-times-circle"></i>
									</div>
									<div class="select_file">
										<input id="passport" type="file" name="passport" data-required-file{if $user->us_citizen==2 && (!$salesflow->application_data['files'] || !$salesflow->application_data['files']['passport'])} required{/if}>
...
<script type="text/javascript" src="design/{$settings->theme}/js/signature_pad.umd.js"></script>
	...
	{/literal}{if $term!='airbnb_short'}{literal}
	var signature = 0;
