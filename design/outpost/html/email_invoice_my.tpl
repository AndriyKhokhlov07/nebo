				...
				<tr>
					<td colspan="4" style="padding: 20px 30px 20px;">
						{if $order->status == 0 || $order->status==4}
							<a style="display:block; border-radius: 7px; background:#008f35; color:#fff; font:500 20px/1 Helvetica; text-align:center; text-transform:uppercase; text-decoration: none; padding: 17px 10px 15px; margin: 0 0 45px;" href="{$config->root_url}/order/{$order->url}?u={$user->id}&w=1" target="_blank">Pay</a>
						{/if}
						
						<p style="color:#666;font:100 13px/1 Helvetica;">You can view invoice status by following this link:<br><a style="color:#000" href="{$config->root_url}/order/{$order->url}?u={$user->id}&w=1" target="_blank">{$config->root_url}/order/{$order->url}?u={$user->id}&w=1</a>
						</p>
					</td>
				</tr>
			</table>
