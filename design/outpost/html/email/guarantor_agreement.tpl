...
{$subject = "{$settings->company_name|escape} | {$user->first_name} {$user->last_name} please sign the Guarantor Agreement" scope=parent}
			...
			<table cellpadding="0" cellspacing="0" border="0" align="center" style="background:#fff; border-radius: 5px; width:100%; box-shadow: 0 3px 15px rgba(0,0,0,.15);">
				<tr>
					<td style="padding: 40px 30px 20px;">
						...
						<p style="color:#333; font: 100 16px/1.3 Helvetica; margin:0 0 15px;">Please sign the Guarantor Agreement.</p>
				...
				<tr>
					<td style="padding: 20px 30px 20px;">
						<a style="display:block; border-radius: 7px; background:#008f35; color:#fff; font:500 20px/1 Helvetica; text-align:center; text-transform:uppercase; text-decoration: none; padding: 17px 10px 15px; margin: 0 0 15px;" href="{$config->root_url}/guarantor/agreement/{$user->auth_code}">Sign</a>
					</td>
				</tr>
				<tr>
					<td style="padding: 0 30px 20px;">
						<p style="color:#333; font:500 12px/1 Helvetica;">
						If the button does not work for you, please use this link:<br> <a style="color:#333; font:500 15px/1 Helvetica;" href="{$config->root_url}/guarantor/agreement/{$user->auth_code}">{$config->root_url}/guarantor/agreement/{$user->auth_code}</a>
						</p>
					</td>
				</tr>
			</table>
