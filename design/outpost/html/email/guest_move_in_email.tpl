{$subject="Your housemates / {$guest->house->name}" scope=parent}
{if $subject_}
			...
			<table cellpadding="0" cellspacing="0" border="0" align="center" style="background:#fff; border-radius: 5px; width:100%; box-shadow: 0 3px 15px rgba(0,0,0,.15);">
				<tr>
					<td colspan="4" style="padding: 40px 30px 20px;">
						<p style="color:#333; font: 500 29px/1 Helvetica; margin:0 0 15px;">Hey {$guest->first_name},</p>
						{foreach $users as $u}{if $u->id != $guest->id}{$u->first_name}{if !$u@last}, {/if}{/if}{/foreach}

						<p style="color:#333; font: 300 17px/1 Helvetica; margin:0 0 15px;">Outpost Club would like to let you know that you will have a new housemate soon! On {$guest->booking->arrive|date}, your new housemate {$guest->name} will arrive and move into {$guest->apt->name} / bed {$guest->bed->name}.
						</p>
