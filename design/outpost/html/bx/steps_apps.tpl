{if $salesflow->type == 1 && in_array($booking->house_id, $airbnb_contracts_houses_ids)}
	{$steps_type='airbnb_contracts'}
	{if $active_step == 1}
		{$next_step_link=false}
	{/if}
{/if}
...
<div class="txt_ center_ inv_questions">
...
{if $next_step_link}
  {$is_params = $next_step_link|strpos:'?'}
	<div class="step fx c">
		<a href="{$next_step_link}{if $is_params}&{else}?{/if}{if $salesflow}s={$salesflow->id}{/if}" class="button_red"{if $timer} data-timer="{$timer}"{/if}>Next step</a>
	</div>
{/if}
...
{* 349 - phila *}
{elseif $steps_type=='airbnb_contracts'}
  {if $active_step>0}
  <div class="steps_sell ch3">
    <div class="item{if $active_step>0} active{/if}">
	    <div class="img">Step 1</div>
...
<div class="steps_sell ch3" {if $booking->client_type_id==2}style="justify-content:center"{/if}>
...
{/if}
    ...
    *}
      {if $booking->client_type_id != 2} {* not airbnb *}
