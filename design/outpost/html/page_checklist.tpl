							...
							<div class="ch_item t{$pb['type']} {if $hide_child}hide_child{/if}">
								...
								{elseif $pb['type']==5} <!-- input -->
									<div class="input_block">
										<input type="hidden" name="name[c{$k}_{$i}]" value="{$pb['title']}">
										<input class="mmr" type="text" name="value[c{$k}_{$i}]" id="c{$k}_{$i}" value="{if $pb['name'] == 'apt' || $pb['name'] == 'bed'}{$guest->$pb['name']->name}{elseif $pb['name']}{$guest->{$pb['name']}}{elseif $mmr["c`$k`_`$i`"]}{$mmr["c`$k`_`$i`"]}{/if}" {if $pb['title']}placeholder="{$pb['title']}"{/if} {if !$hide_child}required{/if}>
										{if $pb['description']}
											<div class="description">{$pb['description']}</div>
										{/if}
									</div>
