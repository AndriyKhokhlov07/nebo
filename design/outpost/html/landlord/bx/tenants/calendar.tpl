    ...
    <div class="c_booking st{$booking->status} fx{if $booking->movein} movein{/if}{if $booking->moveout} moveout{/if}{if $booking->parent_id && ($booking->sp_type==1 || $params->parent_booking->sp_type==1) && $booking->start>0} ext{/if}" style="width:calc({$booking->width}% + 2px);left:{$booking->start}%" data-id="{$booking->id}">
        ...
        <div class="m_info">
            <div class="wrapper">
                        ...
                        <div class="b_title">
                            ...
                            <div class="badge">
                                {if $days_units == 'nights'}
                                    {$booking->days_count-1} {($booking->days_count-1)|plural:'night':'nights'}
                                {else}
                                    {$booking->days_count} {($booking->days_count)|plural:'day':'days'}
                                {/if}
                            </div>
                        </div>
                        <div class="b_val">
                    ...
                    <div class="cont fx">
                        {if $selected_house->type == 1}
                            <div class="column">
                                ...
                                <div class="b_val">
                                    {if $booking->contract && $booking->contract->total_price > 0}
                                        $ {round($booking->contract->total_price / ($booking->days_count-1), 2)}
                                    {elseif $booking->price_night > 0}
                ...
                {if $booking->contract || $booking->airbnb_reservation_id}
                    <div class="bx fx">
                        <div class="icon">
                        ...
                        <div class="cont">
                            ...
                            <div class="b_val s">
                                {if $booking->contract}
                                    #{if $booking->contract->sku}{$booking->contract->sku}{else}{$booking->contract->id}{/if}
                                    <a class="download_doc" href="{$root_url}/files/contracts/{$booking->contract->url}/contract.pdf" target="_blank">
                                        <i class="d_icon"></i>
                                        <span>Download</span>
                                    </a>
                                {elseif $booking->airbnb_reservation_id}
                    ...
                    <div class="bx fx">
                        ...
                        <div class="cont">
                            ...
                            {foreach $booking->note_logs as $l}
                                <div class="b_val b_val_note">{$l->value}</div>
                            {/foreach}
                        </div><!-- cont -->
                    </div><!-- bx -->
                {/if}
...
<div class="tenents_calendar">
    {*<div class="days_header days_bx days_{$params->days_in_month} fx">
        {for $n=1 to $params->days_in_month}
            <div class="d">{$n}</div>
        {/for}
    </div>*}
    <div class="days_lines days_bx days_{$params->days_in_month} fx">
        {for $n=1 to $params->days_in_month}
            <div class="d">
                <span>{$n} </span>
            </div>
