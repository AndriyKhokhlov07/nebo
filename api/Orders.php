...
// 2 => ['name' => 'Order'],
	public $types = array(
		...
		9 => ['name' => 'Fee'],
		10 => ['name' => 'Utility Allowance Fee'],
        16 => ['name' => 'Termination Fee'],
		11 => ['name' => 'Taxes'],
		8 => ['name' => 'Other'],
		12 => ['name' => 'Landlord'],
        13 => ['name' => 'Brokerage'],
	...
	function get_orders($filter = array())
	{
        ...
        elseif (isset($filter['not_type'])) {
            $type_filter = $this->db->placehold('AND o.type NOT in(?@)', (array)$filter['not_type']);
        }
		...
		// dont show old proplem clients (>= 2021-03-23)
		if (isset($filter['future']))
		{
			$order_by = 'date_due';
			$future_filter = $this->db->placehold("AND o.sended=0 AND o.date_due >= '2021-03-24'", $filter['future']);
		}

        if (isset($filter['not_future'])) {
            $future_filter = $this->db->placehold("AND o.date_due<=?", date('Y-m-d'));
        }
		...
		if(isset($filter['payment_method_id']))
			$payment_method_filter = $this->db->placehold('AND o.payment_method_id in(?@)', (array)$filter['payment_method_id']);
		...
		{
			$filter['label'] = (array)$filter['label'];
			$label_join = 'LEFT JOIN __orders_labels AS ol ON o.id=ol.order_id';
			$label_filter = $this->db->placehold('AND ol.label_id in (?@)', $filter['label']);
		}
		if(isset($filter['not_label'])) {
			$filter['not_label'] = (array)$filter['not_label'];
			if (isset($filter['label'])) {
				foreach ($filter['not_label'] as $k=>$nl) {
					if (in_array($nl, $filter['label'])) {
						unset($filter['not_label'][$k]);
					}
				}
			}
			if (!empty($filter['not_label'])) {
				$not_label_select = $this->db->placehold('
					, (select 1 from __orders_labels sub 
					where sub.order_id = o.id and sub.label_id in (?@)
					limit 1
					) as NotCheckedLabel',
					$filter['not_label']
				);
				$not_label_filter = 'HAVING NotCheckedLabel IS NULL';
			}            
        }
        $firstInvInBookFilter = '';
        if (isset($filter['house_id'])) {
            $contract_join = $this->db->placehold('LEFT JOIN __contracts contr ON contr.id = o.contract_id');
            $booking_join = $this->db->placehold('LEFT JOIN __bookings book ON book.id = o.booking_id');
            $house_filter = $this->db->placehold('AND (contr.house_id in (?@) OR book.house_id in (?@))',
			(array)$filter['house_id'], (array)$filter['house_id']);
			if(isset($filter['first_inv_in_book'])){
				$firstInvInBookFilter = $this->db->placehold('AND o.date_from = book.arrive');
			}
        }

        /** @var DatesOrderModel $datesModel */
        if ($datesModel = DatesOrderModel::getFromFilter($filter)) {
            $dates_filter = $datesModel->getSqlFilter();
        }

		if(!empty($filter['keyword']))
		{
            $keyword_join = $this->db->placehold('
                LEFT JOIN __orders_users ou ON o.id=ou.order_id
                LEFT JOIN __users u ON ou.user_id=u.id
            ');
			$keywords = explode(' ', $filter['keyword']);
			foreach($keywords as $keyword)
				$keyword_filter .= $this->db->placehold('AND (o.id = "'.$this->db->escape(trim($keyword)).'" OR u.name LIKE "%'.$this->db->escape(trim($keyword)).'%" OR REPLACE(u.phone, "-", "")  LIKE "%'.$this->db->escape(str_replace('-', '', trim($keyword))).'%" OR u.email = "'.trim($keyword).'" OR o.address LIKE "%'.$this->db->escape(trim($keyword)).'%" OR o.sku = "'.$this->db->escape(trim($keyword)).'" )');
		}
		...
		// upd
		if(isset($filter['date_created_month']) && isset($filter['date_created_year']))
		{
			$or_paid_month = '';
			if(isset($filter['or_paid_month']))
			{
				$or_paid_month = $this->db->placehold(' OR (MONTH(o.payment_date)=? AND YEAR(o.payment_date)=?)', $filter['date_created_month'], $filter['date_created_year']);
			}

			$debt = '';
			if(isset($filter['debt']))
				$debt = $this->db->placehold(' OR (o.paid=0 AND o.date<=?)', $filter['date_created_month'].'-'.$filter['date_created_year'].'-01' );

			$date_created_filter = $this->db->placehold(' AND ((MONTH(o.date)=? AND YEAR(o.date)=?) '.$or_paid_month.$debt.')', $filter['date_created_month'], $filter['date_created_year']);
		}
		// ---------
		...
		if(isset($filter['date_month_from_to']) && isset($filter['date_year_from_to']))
		{
			...
			if(isset($filter['or_paid_month']))
			{
				...
				/*$or_paid_month_type = '';
				if(isset($filter['type']))
					$or_paid_month_type = $this->db->placehold('AND o.type in(?@)', (array)$filter['type']);

				$or_paid_month = $this->db->placehold(" OR (MONTH(o.payment_date)=? AND YEAR(o.payment_date)=? $or_paid_month_type)", $filter['date_from_month'], $filter['date_from_year']);*/
	...
	function count_orders($filter = array())
	{
		...
		if(isset($filter['not_label'])) {
			...
			if (!empty($filter['not_label'])) {
				$not_label_filter = $this->db->placehold('
                AND (select 1 from __orders_labels sub 
                    where sub.order_id = o.id and sub.label_id in(?@)
                    limit 1) IS NULL',
                $filter['not_label']
            );
	...
	public function update_order($id, $order)
	{
        ...
        if ($order['paid'] == 1 && $order_->status != 3) {
            $order['status'] = 2;
        }
		...
		if(!empty($order_->booking_id))
		{
			...
			if ($order['deposit'] == 1 || $order_->deposit == 1) {
				if (!empty($order['status']) && $order['status'] != $order_->status) {
					...
					foreach ($salesflows as $salesflow) {
						...
						if($order['status'] == 2) {
                            // Auto approve for The Mason on Chestnut
                            if ($booking->house_id == 349) {
                                $this->salesflows->approve_salesflow([
			...
			elseif((!empty($order['type']) && $order['type'] == 1) || (empty($order['type']) && $order_->type == 1))
			{
	...
	public function add_order($order)
	{
		...
		if(empty($order->date) && empty($order->date_due))
		{
			...
			{
				date_add($date_creation, date_interval_create_from_date_string('1 days'));
	...
	*/
	...
	private function update_total_price($order_id)
	{
		...
		if(empty($order))
			return false;
		if($order->discount_type == 2) // summ
		{
			$query = $this->db->placehold("UPDATE __orders o SET o.total_price=IFNULL((SELECT SUM(p.price*p.amount) FROM __purchases p WHERE p.order_id=o.id), 0)+o.delivery_price*(1-o.separate_delivery)-o.coupon_discount-o.discount, modified=NOW() WHERE o.id=? LIMIT 1", $order->id);
		}
		else
		{
