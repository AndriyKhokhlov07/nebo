...
class Image extends Backend
{
	...
	{
		list($source_file, $type, $width , $height, $set_watermark, $grayscale) = $this->get_resize_params($filename, $type);
		...
		{	
			...
			$resized_file = $this->add_resize_params($original_file, $type, $width, $height, $set_watermark, $grayscale);			
		}	
		else
		{
		...
		elseif($type == 'category'){
			...
			$preview_dir   = $this->config->root_dir.$this->config->resized_categories_images_dir;
		}
		elseif($type == 'brand'){
		...
		elseif($type == 'blog'){
			$originals_dir = $this->config->root_dir.$this->config->blog_images_dir;
			$preview_dir   = $this->config->root_dir.$this->config->resized_blog_images_dir;
		}
		elseif($type == 'pages'){
			$originals_dir = $this->config->root_dir.$this->config->pages_images_dir;
			$preview_dir   = $this->config->root_dir.$this->config->resized_pages_images_dir;
		}
		elseif($type == 'loans'){
			$originals_dir = $this->config->root_dir.$this->config->loans_images_dir;
			$preview_dir   = $this->config->root_dir.$this->config->resized_loans_images_dir;
		}
		elseif($type == 'gallery'){
			$originals_dir = $this->config->root_dir.$this->config->galleries_images_dir;
			$preview_dir   = $this->config->root_dir.$this->config->resized_galleries_images_dir;
		}
		elseif($type == 'user'){
			$originals_dir = $this->config->root_dir.$this->config->users_images_dir;
			$preview_dir   = $this->config->root_dir.$this->config->resized_users_images_dir;
		}
		...
		if(class_exists('Imagick') && $this->config->use_imagick)
			$this->image_constrain_imagick($originals_dir.$original_file, $preview_dir.$resized_file, $width, $height, $watermark, $grayscale, $watermark_offet_x, $watermark_offet_y, $watermark_transparency, $sharpen);
		else
			$this->image_constrain_gd($originals_dir.$original_file, $preview_dir.$resized_file, $width, $height, $watermark, $grayscale, $watermark_offet_x, $watermark_offet_y, $watermark_transparency);
		return $preview_dir.$resized_file;
	}

	public function add_resize_params($filename, $type, $width=0, $height=0, $set_watermark=false, $grayscale=false)
	{
		...
		if($width>0 || $height>0)
			$resized_filename = $file.'.'.($width>0?$width:'').'x'.($height>0?$height:'').($set_watermark?'w':'').($grayscale?'g':'').'.'.$ext;
		else
			$resized_filename = $file.'.'.($set_watermark?'w.':'').($grayscale?'g':'').$ext;
			
		return $resized_filename;
	}

	public function get_resize_params($filename, $type)
	{
		// Определаяем параметры ресайза
		if(!preg_match('/(.+)\.([0-9]*)x([0-9]*)(w)?(g)?\.([^\.]+)$/', $filename, $matches))
			return false;
		...
		$grayscale = $matches[5];				// насыщенность (ч/б)
		$ext = $matches[6];						// расширение файла
		$type = $type;
			
		return array($file.'.'.$ext, $type, $width, $height, $set_watermark, $grayscale);
	}
	...
	public function upload_image($filename, $name, $type='gallery')
	{
		// Имя оригинального файла
		...
		if($type == 'product')
			$uploaded_dir = $this->config->root_dir.$this->config->original_images_dir;
		elseif($type == 'gallery')
			$uploaded_dir = $this->config->root_dir.$this->config->galleries_images_dir;
		elseif($type == 'inventories')
			$uploaded_dir = $this->config->root_dir.$this->config->inventories_images_dir;
		elseif($type == 'cleanings')
			$uploaded_dir = $this->config->root_dir.$this->config->cleaning_images_dir;
		elseif($type == 'user')
			$uploaded_dir = $this->config->root_dir.$this->config->users_images_dir;
		elseif($type == 'tmp')
			$uploaded_dir = $this->config->root_dir.$this->config->tmp_dir;
		
		if(in_array(strtolower($ext), $this->allowed_extentions))
		{			
			while(file_exists($uploaded_dir.$new_name))
			{	
				$new_base = pathinfo($new_name, PATHINFO_FILENAME);
				if(preg_match('/_([0-9]+)$/', $new_base, $parts))
					$new_name = $base.'_'.($parts[1]+1).'.'.$ext;
			...
			if(move_uploaded_file($filename, $uploaded_dir.$new_name))			
				return $new_name;
		}
	 ...
	 * @return bool
	 */
	private function image_constrain_gd($src_file, $dst_file, $max_w, $max_h, $watermark=null, $grayscale=null, $watermark_offet_x=0, $watermark_offet_y=0, $watermark_opacity=1)
	{
		$quality = 100;
		...
		// Нужно ли обрезать?
		if (!$watermark && !$grayscale && ($src_w <= $max_w) && ($src_h <= $max_h)){
	 ...
	 * @return bool
	 */
	private function image_constrain_imagick($src_file, $dst_file, $max_w, $max_h, $watermark=null, $grayscale=null, $watermark_offet_x=0, $watermark_offet_y=0, $watermark_opacity=1, $sharpen=0.2)
	{
		$thumb = new Imagick();
