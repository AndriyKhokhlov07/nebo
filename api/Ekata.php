...
';		
					...
					$salesflow->ekata_status['ekata_network_score'] = $result->identity_network_score;
					$salesflow->ekata_status['ekata_check_score'] = $result->identity_check_score;
					if(!empty($user->phone))
					{
						if($result->primary_phone_checks->is_valid != 1)
							$salesflow->ekata_status['ekata_phone_check'] = 'not_valid';
						else
							$salesflow->ekata_status['ekata_phone_check'] = strtolower($result->primary_phone_checks->line_type);
					}
					...
					$this->salesflows->update_salesflow($user->active_salesflow_id, ['ekata_status' => serialize($salesflow->ekata_status)]);
