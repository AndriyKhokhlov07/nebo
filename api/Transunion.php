...
class Transunion extends Backend
{
	...
	private $password = 'q?T289j799.WW.QQ.55';
	...
	public function create_application($user_id)
	{
		...
		$this->guest_info = $this->users->get_users([
			'id' => (int)$user_id, 
			'salesflow_info' => 1,
			'count' => 1
		]);
		...
		if($this->guest_info->status == 10) // Guarantor
		{
			$uu = $this->users->get_users_users([
				'type' => 1, // Guarantor
				'child_id' => $this->guest_info->id,
			...
			if(!empty($uu))
			{
				$tenant = $this->users->get_user((int)$uu->parent_id);
				if(!empty($tenant))
				{
					$this->guest_info->active_booking_id = $tenant->active_booking_id;
				}
			}
		}
		...
		elseif($booking->type == 2) // apartment booking
		{
			...
			{
				if(empty($this->guest_info->price_month) && $booking->apartment->price > 0)
				{
		...
		if(!empty($contracts))
		{
			...
			if(!empty($contract))
			{
				...
				if(empty($this->guest_info->price_deposit) || $this->guest_info->price_deposit < 1)
		...
		if(!empty($res))
		{
			...
			if($result->status->statusCode == 1)
			{
				...
				$salesflow_upd->transunion_id = $result->application->applicationNumber;
				$salesflow_upd->transunion_status = $result->application->scoreResult->applicationRecommendation;
				$salesflow_upd->transunion_data = $res;
				...
				$this->salesflows->update_salesflow($this->guest_info->active_salesflow_id, $salesflow_upd);
