...
class Occupancy extends Backend
{
	...
	public function init_occupancy($params = [])
	{
				...
				if(!empty($apartments_ids) && isset($this->params->houses[$house_id]))
				{
					...
					foreach($apartments_ids as $apartment_id)
					{
						...
						if(isset($this->params->apartments[$apartment_id]))
						{
							...
							if(($a->house_id == 334 || $a->house_id == 337) && $params['landlord_view'] == 1)
							{
								$a->date_added = 0;
							}
			...
			if(!empty($bookings_))
			{
				...
				foreach($bookings as $b_id=>$b)
				{
					...
					*/
					...
					$b->days_count = ceil($b->interval / (24 * 60 * 60));
					...
					if(!is_null($this->params->apartments[$b->apartment_id]->date_added))
					{
						$apartment_u_added = strtotime($this->params->apartments[$b->apartment_id]->date_added);
					...
					if(!is_null($this->params->apartments[$b->apartment_id]->date_shutdown))
					{
						$apartment_u_shutdown = strtotime($this->params->apartments[$b->apartment_id]->date_shutdown);
		...
		foreach($this->params->houses as $h)
		{
			...
			if(empty($this->params->houses[$h->id]->days_beds_count) && !empty($this->params->houses[$h->id]->beds_tcount) && !empty($this->params->houses[$h->id]->occupancy_bdays))
			{
				$this->params->houses[$h->id]->occupancy2 = ceil($this->params->houses[$h->id]->occupancy_bdays / $this->params->houses[$h->id]->beds_tcount * 100);
			}
