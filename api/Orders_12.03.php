...
class Orders extends Backend
{
	...
	function get_orders($filter = array())
	{
		...
		{
			$order_by = 'date';
			$future_filter = $this->db->placehold('AND (o.date>=? AND o.sended=0)', $filter['future']);
		}
		
		if(isset($filter['past']))
			$future_filter = $this->db->placehold('AND (o.date <= CURDATE() OR o.sended=1)');
		...
		if(!empty($filter['keyword']))
		{
			...
			foreach($keywords as $keyword)
				$keyword_filter .= $this->db->placehold('AND (o.id = "'.$this->db->escape(trim($keyword)).'" OR o.name LIKE "%'.$this->db->escape(trim($keyword)).'%" OR REPLACE(o.phone, "-", "")  LIKE "%'.$this->db->escape(str_replace('-', '', trim($keyword))).'%" OR o.email = "'.trim($keyword).'" OR o.address LIKE "%'.$this->db->escape(trim($keyword)).'%" )');
		}

		if(isset($filter['date_from_month']) && isset($filter['date_from_year']))
		{
			$or_paid_month = '';
			if(isset($filter['or_paid_month']))
				$or_paid_month = $this->db->placehold('OR (MONTH(o.payment_date)=? AND YEAR(o.payment_date)=?)', $filter['date_from_month'], $filter['date_from_year']);


			$date_from_filter = $this->db->placehold('AND ((MONTH(o.date_from)=? AND YEAR(o.date_from)=?) '.$or_paid_month.')', $filter['date_from_month'], $filter['date_from_year']);
		}
	...
	public function add_order($order)
	{
		...
		if(empty($order->date))
		{
			...
			date_add($date_creation, date_interval_create_from_date_string('2 days'));
			$set_curr_date = ', date=now(), date_due="'.$date_creation->format('Y-m-d').'"';
		}
		else
		{
