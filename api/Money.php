...
class Money extends Backend
{
	...
	public function __construct()
	{
        ...
        if(isset($this->settings->price_position))
            $this->position = $this->settings->price_position;
	...
	private function init_currencies()
	{
		...
		$query = "SELECT id, name, sign, code, rate_from, rate_to, cents, position, sorting, enabled FROM __currencies ORDER BY sorting";
		$this->db->query($query);
        ...
        $GLOBALS['view']->currency = $this->currency;
	}
	...
	{	
		...
		$this->db->query("UPDATE __currencies SET sorting=id WHERE id=?", $id);
		$this->init_currencies();
